  ...
  return (
    <div>
      ...
      <header className={activeMenu ? 'hidden' : 'fixed flex md:justify-between z-20 p-4 dark:bg-primary bg-gray-100 shadow-lg text-lg md:items-center  rounded flex-col md:flex-row gap-4 w-[250px] md:w-full top-16 left-2 md:static  '}>
        ...
        <nav>
          <ul className='flex md:flex-row md:gap-5 flex-col gap-7 '>
            {navLinks.map((link, index) => (
              <li key={index}>
                <NavLink
                  to={link.to}
                  className={({ isActive }) =>
                    isActive ? 'font-bold bg-indigo-700 w-[300px] h-[300px] p-2  text-white text-sm' : ' font-bold text-sm dark:text-[#7777c2] text-gray-500 p-2 dark:hover:bg-indigo-900 hover:bg-gray-200  '}
                >
                  {link.text}
                </NavLink>
              </li>
            ...
            <button
              ...
              className='dark:hover:bg-indigo-900 hover:bg-gray-200 hover:rounded'
            >
      ...
      <footer className='dark:bg-primary p-10  flex justify-between md:h-[300px] bg-gray-200 shadow-lg flex-col gap-10 md:flex-row items-center border-[1px] border-b-0 border-gray-800'>
        ...
        <p>Derechos reservados by Monsster</p>
